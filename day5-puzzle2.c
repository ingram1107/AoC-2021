...
void foundHydrothermalVentDiagnonal(int *diagram, int firstPosition, int secondPosition, int width)
{
  int distance = firstPosition - secondPosition;

  if (distance % (width+1) == 0) {
    for (int i = secondPosition; i < firstPosition+1; i += width+1)
      diagram[i]++;
  } else if (distance % (width-1) == 0) {
    for (int i = secondPosition; i < firstPosition+1; i += width-1)
      diagram[i]++;
  }
}
...
int main(void)
{
  ...
  while (fgets(line, BUFSIZE, input) != 0) {
    ...
    int positionLast = lastX*width + lastY, positionNext = nextX*width + nextY;
    if (lastX == nextX) {
      if (lastY > nextY) {
    ...
    } else if (positionLast > positionNext) {
      foundHydrothermalVentDiagnonal(hydrothermalVentsDiagram, positionLast, positionNext, width);
    } else if (positionLast < positionNext) {
      foundHydrothermalVentDiagnonal(hydrothermalVentsDiagram, positionNext, positionLast, width);
    }
  }
